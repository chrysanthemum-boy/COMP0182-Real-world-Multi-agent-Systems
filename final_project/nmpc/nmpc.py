...
def simulate(filename):
    ...
    start_positions = [np.array([5, 5]), np.array([5, 5])]
    goals = [np.array([5, 5]), np.array([5, 5])]

    robot_states = start_positions
    robot_histories = [np.empty((4, NUMBER_OF_TIMESTEPS)),
                       np.empty((4, NUMBER_OF_TIMESTEPS))]
    ...
    for i in range(NUMBER_OF_TIMESTEPS):
        for robot_index in range(len(robot_states)):
            ...
            p_desired = goals[robot_index]
            obstacle_predictions = predict_obstacle_positions(obstacles[:, i, :])
            robot_state_history = robot_histories[robot_index]
    ...
    plot_robots_and_obstacles(
        robot_histories, obstacles, ROBOT_RADIUS, NUMBER_OF_TIMESTEPS, SIM_TIME, filename)
