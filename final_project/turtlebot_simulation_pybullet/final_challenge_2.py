...
def create_agents(yaml_file):
    ...
    for agent in agent_yaml_params["agents"]:
        ...
        if first == True:
        	box_id = p.loadURDF("./data/turtlebot1.urdf", start_position, start_orientation, globalScaling=1.2)
        	first = False
        else:
            box_id = p.loadURDF("data/turtlebot2.urdf", start_position, start_orientation, globalScaling=1.2)
...
cbs_schedule = read_cbs_output("./final_challenge/cbs_output_fixed.yaml")
